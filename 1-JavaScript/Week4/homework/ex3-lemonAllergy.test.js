...
// ! Function under test
function sanitizeFruitBasket(basket, fruitsName) {
  return basket.filter(item => item !== fruitsName);
}
...
describe('sanitizeFruitBasket', () => {
  ...
  test('should not modify the original `fruitBasket` array', () => {
    ...
    expect(JSON.stringify(fruitBasket)===JSON.stringify(originalFruitBasketContents)).toBe(true);
  });

  test('should return a new array that does not include the unwanted `lemon`', () => {
    sanitizeFruitBasket = sanitizeFruitBasket(fruitBasket, 'lemon');
    expect(sanitizeFruitBasket.includes('lemon')===false).toBe(true);
  });
});
